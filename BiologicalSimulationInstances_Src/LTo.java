 ...
 *
 */
public class LTo extends SimplePortrayal2D implements Steppable,Stoppable
{
	/**
    ...
    public void incImLToCellContactCount()
    {
    	this.imLToCellContactCount++;
    }
	 ...
	 * <b>Link to Domain and Platform Models:</b>
	 */
	public double immatureActiveTime;
	...
	/**
     * Creates a new LTo Agent (or Cell), and assigns the cell a linear adjustment and CCL19 Expression level.
     ...
     */
    public LTo(Double2D location,Int2D gridLocation, int ltoNum,Boolean ARTNExpression,double cellActiveTime,ArrayList<ArrayList<String>> cellExpressors) 
    {
    	this.agentLocation = location;
        
        this.immatureActiveTime = cellActiveTime;
       ...
       // Set up the expressors for this object
        ...
        //this.startingVCAMExpressionLevel = this.vcamAdhesionEffect.vcamExpressionLevel * sp.adhesionFactorExpressionSlope;
    ...
    public LTo(Double2D location,Int2D gridLocation, int ltoNum,Boolean ARTNExpression,double cellActiveTime,ArrayList<Object> cellExpressors,Boolean replicate)
    {
    	this.agentLocation = location;
    	
        this.immatureActiveTime = cellActiveTime;
        
        // Just copy the receptors as this is a replicate of another LTo
        this.expressors = cellExpressors;
    ...
    public void stableContact(Object contactedCell,int timeStep)
    {
    	if(contactedCell.getClass().getName().equals("LTin"))
		{
			...
			// If an adhesion factor is being expressed by the cell, increase adhesion expression
			// Do however need to find the receptor
			// Set up the receptors for this object
	        
			for(int i=0;i<this.expressors.size();i++)
	    	{
	    			
	    			try
	    	    	{
	    				// get the receptor
	    				Object receptor = this.expressors.get(i);
	    				Method meth = receptor.getClass().getMethod("incrementAdhesionExpression",new Class[]{});
	    				meth.invoke(receptor);
	    	    	}
		...
		else
		{
			...
			//if(this.chemoLinearAdjust>simParams.maxChemokineExpressionValue)
			//{
				//this.chemoLinearAdjust-=simParams.chemoLinearAdjustmentReducer;
			//}
		
			// increase level of chemokine expression by LTo - a chemokine is being expressed
			...
			for(int i=0;i<this.expressors.size();i++)
    		{
				...
				{
	    			// get the receptor
	    			Object expressor = this.expressors.get(i);
	    			
	    			Class<?> cellType = expressor.getClass();
					Method meth = cellType.getMethod("increaseChemokineExpression",new Class[]{});
					meth.invoke(expressor);
				}
				catch(NoSuchMethodException e)
				{
					//e.printStackTrace();
    ...
    public void step( final SimState state )
    {
    	...
    	if(ppsim.schedule.getSteps()*ppsim.simulationSpec.secondsPerStep < ((ppsim.simulationSpec.simulationTime*60)*60))
		{
    		if(!this.stopped)
    		{
    			...
    			if(this.cellState==0 && this.activeTime > ((this.immatureActiveTime*60)*60))
    			{
    				this.stopped=true;
    			...
    			else
    			{
    				...
    				if(this.cellState==0)
    				{
    					...
    					if(this.imLToCellContactCount>this.numContactsActivateLTo)
    					{
    	...
    	else
    	{
    		...
    		//	catch(Exception e)
    		//	{
    				// Just catch potential exception
    		//	}
		...
		if(!convertDone)
		{
			if(blankSpace!=null)
			{
				...
				try
				{
					...
					Constructor<?> con = this.getClass().getConstructor(new Class[]{Double2D.class, Int2D.class,int.class,Boolean.class,double.class,ArrayList.class,Boolean.class});
					Object newCellObject = con.newInstance(location,blankSpace,ppsim.ltoCellsBag.size(),true,this.immatureActiveTime,this.expressorNames,true);
    ...
    /**
     * Used by the Graphical Display to draw the cell on the screen
     */
    public boolean hitObject(Object object, DrawInfo2D info)
    {
    	// The diameter of the object will be set dependent on whether the user has zoomed in.  Therefore, 
		// two variables are needed to calculate the size
		double diamx;
		double diamy;
		// work out what cell type is being drawn
		diamx = info.draw.width*cell_diameter;
		diamy = info.draw.height*cell_diameter;
    ...
    public final void draw(Object object, Graphics2D graphics, DrawInfo2D info)
    {
        ...
        double diamx = info.draw.width*cell_diameter;
        double diamy = info.draw.height*cell_diameter;
